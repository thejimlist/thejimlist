...
<html>
	<head>
		<meta charset="utf-8"/>
		<title>Get Ready for IPS Community Suite 4.0</title>
...
<?php if ( isset( $_GET['phpinfo'] ) ) { phpinfo(); exit; } ?>
		...
		<div id='main'>
<?php $success = TRUE; $nexus = TRUE; $installOnly = TRUE; ?>
			<section>
...
<?php else: $success = FALSE; ?>
					<li class="fail">You are not running a compatible version of PHP. You need PHP 5.3.0 or above. You should contact your hosting provider or system administrator to ask for an upgrade.</li>
<?php endif; ?>
...
<?php
	...
	$installOnly = FALSE;
?>
<?php if ( version_compare( $mysql->server_info, '5.6.0' ) >= 0 ): ?>
						<li class="success">MySQL version <?php echo $mysql->server_info; ?>.</li>
<?php elseif ( version_compare( $mysql->server_info, '5.1.3' ) >= 0 ): ?>
						<li class="advisory">You are running MySQL version <?php echo $mysql->server_info; ?>.<p class='smaller'>While this version is compatible, we recommend version 5.6.0 or above, but this is not essential. You may wish to contact your hosting provider or system administrator to ask for an upgrade if you are upgrading to IPS4.</p></li>
<?php else: $success = FALSE; ?>
						<li class="fail">You are not running a compatible version of MySQL. You need MySQL 5.1.3 or above. You should contact your hosting provider or system administrator to ask for an upgrade.</li>
<?php endif; ?>
<?php $utf8 = TRUE; $tables = $mysql->query( "SHOW TABLES LIKE '{$INFO['sql_tbl_prefix']}%';" ); while ( $table = $tables->fetch_assoc() ): ?>
<?php $tableName = array_pop( $table ); $columns = $mysql->query( "SHOW FULL COLUMNS FROM `{$tableName}`;" ); while ( $column = $columns->fetch_assoc() ): ?>
<?php if ( substr( $tableName, 0, 5 ) !== 'orig_' and $column['Collation'] and !in_array( $column['Collation'], array( 'utf8_unicode_ci', 'utf8mb4_unicode_ci' ) ) ) { $utf8 = FALSE; $utf8Info = "{$tableName}.{$column['Field']} is {$column['Collation']}"; break 2; } ?>
<?php endwhile; ?>
<?php endwhile; ?>
<?php if ( $utf8 ): ?>
						<li class="success">All database tables UTF8.</li>
<?php else: $success = FALSE; ?>
						<li class="fail">Some or all of the columns in your database are not using the utf8_unicode_ci collation (<?php echo $utf8Info; ?>). <a href="http://community.invisionpower.com/files/file/6823-ips-utf8-database-converter/" target="_blank">Download the UTF8 Database Converter</a></li>
<?php endif; ?>
<?php  else: $mysql = @new my_mysqli( 'localhost' ); if ( $mysql->connect_errno ): ?>
						<li class="advisory">MySQL connection could not be established to perform version check. Make sure your MySQL Server version is 5.1.3 or above (5.6.0 or above recommended).</li>
<?php else: ?>
<?php if ( version_compare( $mysql->server_info, '5.6.0' ) >= 0 ): ?>
						<li class="success">MySQL version <?php echo $mysql->server_info; ?>.</li>
<?php elseif ( version_compare( $mysql->server_info, '5.1.3' ) >= 0 ): ?>
						<li class="advisory">You are running MySQL version <?php echo $mysql->server_info; ?>. While this version is compatible, we recommend version 5.6.0 or above. You may wish to contact your hosting provider or system administrator to ask for an upgrade.</li>
<?php else: $success = FALSE; ?>
						<li class="fail">You are not running a compatible version of MySQL. You need MySQL 5.1.3 or above. You should contact your hosting provider or system administrator to ask for an upgrade.</li>
<?php endif; ?>
<?php endif; endif; ?>
...
<?php else: ?>
					<li class="advisory">Your license is currently inactive. You will need to <a href="http://www.invisionpower.com/clients" target="_blank">renew</a> before you can upgrade.</li>
<?php endif; ?>
<?php endif; ?>
				</ul>
			</section>
			...
			<section id="summary">
				<h1>Summary</h1>
<?php if ( $success ): ?>
				<p class="success"><?php if ( $nexus ): ?>You are ready to install IPS Community Suite 4.0!<?php else: ?>You are ready to install IPS Community Suite 4.0, but not IP.Nexus.<?php endif; ?></p>
<?php if ( $installOnly ): ?>
...
<?php endif; ?>
<?php else: ?>
				<p class="fail">You are not ready to upgrade to IPS Community Suite 4.0 yet. See the information above for instructions how to fix or <a href="http://www.invisionpower.com/clients" target="_blank">contact technical support</a> for further assistance.</p>
<?php endif; ?>
			</section>
		</div>
	</body>
